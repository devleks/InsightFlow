...
from langchain_community.vectorstores.utils import filter_complex_metadata # Added for ChromaDB compatibility
    ...
    try:
        if source_type == 'pdf':
            ...
            loader = DirectoryLoader(
                ...
                loader_kwargs={'mode': 'elements'} # Instruct UnstructuredFileLoader to use 'elements' mode
            )
            ...
            for ext_type in SUPPORTED_EXTENSIONS:
                ...
                try:
                    ...
                    if loaded_docs_for_type and ext_type == ".pdf":
                        # st.write("--- DEBUG: Metadata from DirectoryLoader for first loaded PDF doc BEFORE splitting ---")
                        st.write(f"Source: {loaded_docs_for_type[0].metadata.get('source')}")
                        # st.write(loaded_docs_for_type[0].metadata)
                        # st.write("--- END DEBUG --- LATEST")
                        pass # Keep the if block structure, do nothing if prints are commented
        ...
        # Add length_function if needed, e.g., len for character count
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=chunk_size,
            chunk_overlap=chunk_overlap,
            # length_function=len, # Example: Use if your text units are characters
        ...
        if doc_splits:
            # Apply metadata filtering before returning
            # This is to remove complex data types that ChromaDB can't handle from Unstructured mode='elements'
            filtered_splits = filter_complex_metadata(doc_splits)

            first_pdf_split = next((split for split in filtered_splits if split.metadata.get('source', '').endswith('.pdf')), None)
            if first_pdf_split:
                # st.write("--- DEBUG: Metadata of first PDF chunk AFTER splitting AND filtering ---")
                # st.write(f"Source: {first_pdf_split.metadata.get('source')}")
                # st.write(first_pdf_split.metadata)
                # st.write("--- END DEBUG --- LATEST")
                pass # Keep the if block structure, do nothing if prints are commented
            return filtered_splits # Return filtered splits
        else:
            ...
            with st.chat_message(message["role"], avatar=avatar_icon):
                ...
                if message["role"] == "assistant" and "sources" in message and message["sources"]:
                    ...
                    for i, source_meta in enumerate(message["sources"]):
                        ...
                        page_num = source_meta.get('page_number', source_meta.get('page', 'N/A'))
                ...
                with st.chat_message("assistant", avatar="🤖"): 
                    ...
                    try:
                        with st.spinner("Thinking...",show_time=True):
                                ...
                                for doc in source_docs:
                                    if hasattr(doc, 'metadata'):
                                        ...
                                        clean_meta = {
                                            ...
                                            'page': meta.get('page_number', meta.get('page')) # Keep as is (likely int/None/str)
                            ...
                            # Display the message content and sources immediately in the placeholder
                            full_response_display = assist_msg
                            if source_metadata_list:
                                # Using <br> for spacing, can be adjusted with CSS later if needed
                                sources_html = "<br><br><div style='font-size: 0.9em; color: #888;'><strong>Sources:</strong><ul>"
                                for source_info in source_metadata_list:
                                    source_name = os.path.basename(source_info.get('source', 'Unknown'))
                                    page_number = source_info.get('page_number', source_info.get('page', 'N/A'))
                                    sources_html += f"<li>{source_name} (Page: {page_number})</li>"
                                sources_html += "</ul></div>"
                                full_response_display += sources_html
